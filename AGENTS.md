...
- **SQLite writer coordination is database-wide and cleanup is build-aware.** Every mutation on
  ...
  database file so relative/symlink aliases cannot bypass the gate. Each `:memory:` database gets
  an independent coordination object and must not create or rely on a filesystem build lock.
